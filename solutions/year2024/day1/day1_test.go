...
func TestRunPartOne(t *testing.T) {
	...
	t.Run("PartTwo with nil file", func(t *testing.T) {
		got := RunPartTwo(nil, 2024, 1)
		if got != 0 {
			t.Errorf("got %v want %v", got, nil)
		}
	})
	...
	t.Run("PartOne with Test Data from AOC", func(t *testing.T) {
	...
	//Part Two Test
	t.Run("PartTwo with Test Data from AOC", func(t *testing.T) {
		lines := []string{"3   4", "4   3", "2   5", "1   3", "3   9", "3   3"}
		got := RunPartTwo(lines, 2024, 1)
		want := 31
		if got != want {
			t.Errorf("got %v want %v", got, want)
		}
